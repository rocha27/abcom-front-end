...
@Component({
    ...
    templateUrl: './item-alterar.component.html'
})
export class ItemAlterarComponent implements OnInit {
    ...
    alterar(produto: Produto) {
        this.produtoService.editar(produto).then(result=> {
            this.appMessageService.addMsgSuccess(this.translateService.instant('Alterado com sucesso!'));
            window.history.back();
        }).catch(erro=> {
            console.log(erro);
        })
